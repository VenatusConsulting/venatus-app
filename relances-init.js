import { getRelances, getLead, updateLead, addNote } from "./api-module.js";
...
function renderList(leads, containerId) {
  ...
  if (!leads.length) { el.innerHTML = '<div class="empty">Aucun lead ici 🎉</div>'; return; }
  el.innerHTML = leads.map(lead => {
    const s = STATUTS[lead.statut] || STATUTS.nouveau;
    const retard = getRetard(lead.date_relance);
    return `
      <div class="lead-row" onclick="window._openLeadR('${lead._id}')">
        <div class="lead-main">
      ...
      <div class="statut-buttons">
        ${Object.entries(STATUTS).map(([key, val]) => `
          <button class="btn-statut ${lead.statut === key ? 'active' : ''}" style="--color:${val.color}"
            onclick="window._changeStatutR('${key}')">${val.label}</button>
        `).join("")}
      </div>
    </div>
    <div class="modal-section">
      ...
      <div class="note-input-wrap">
        ...
        <button onclick="window._saveNoteR()" class="btn-primary">Ajouter</button>
      </div>
    </div>
  `;
}

window._openLeadR = openLead;
window._changeStatutR = async (statut) => {
  await updateLead(currentId, { statut });
  window.closeModal();
  window.initRelances();
};
window._saveNoteR = async () => {
...
window.initRelances = async function() {
