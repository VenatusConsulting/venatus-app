import { getStatsDM } from "./api-module.js";
...
window.initStats = async function() {
  ...
  document.getElementById("dm-table").innerHTML = `
    <table class="dm-table">
      <thead><tr><th>DM</th><th>Envoyés</th><th>Réponses</th><th>Signés</th><th>Taux</th><th>Rang</th></tr></thead>
      <tbody>
        ${sorted.map(([key, val], i) => `
          <tr>
            ...
            <td><span class="taux-badge" style="color:${val.taux >= 30 ? 'var(--green)' : val.taux >= 15 ? 'var(--yellow)' : 'var(--text3)'}">${val.taux}%</span></td>
  ...
  document.getElementById("dm-templates").innerHTML = Object.entries(DM_TEMPLATES).map(([key, val]) => {
    const d     = data[key] || { total: 0, reponses: 0, signes: 0, taux: 0 };
    const color = DM_COLORS[key];
    return `
      <div class="template-card">
        <div class="template-header">
          <span class="template-label" style="color:${color}">${val.label}</span>
          <span class="template-taux" style="color:${d.taux >= 30 ? 'var(--green)' : d.taux >= 15 ? 'var(--yellow)' : 'var(--text3)'}">${d.taux}% réponses</span>
        </div>
