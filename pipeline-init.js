...
const NICHES = { influenceuse:"💋 Influenceuse", fitness:"💪 Fitness", gaming:"🎮 Gaming", cosplay:"🎨 Cosplay" };

let currentId = null, draggedId = null;
...
function renderKanban(leads) {
  ...
  for (const lead of leads) { if (byStatut[lead.statut]) byStatut[lead.statut].push(lead); }
  ...
  kanban.innerHTML = Object.entries(STATUTS).map(([key, val]) => {
    ...
    const cards = list.map(lead => {
      ...
      return `
        <div class="kanban-card ${retard ? 'retard' : ''}"
          onclick="window._openLeadP('${lead._id}')"
          draggable="true"
          ondragstart="window._dragStart(event,'${lead._id}')"
        >
          ...
          ${retard ? `<div class="retard-badge">⚠️ ${retard}j de retard</div>`
            : lead.date_relance ? `<div class="relance-badge">⏰ ${lead.date_relance}</div>` : ""}
        </div>
      `;
    }).join("");

    return `
      <div class="kanban-col" ondragover="event.preventDefault()" ondrop="window._dragDrop(event,'${key}')">
...
window._dragStart = (event, id) => { draggedId = id; event.dataTransfer.effectAllowed = "move"; };
window._dragDrop  = async (event, newStatut) => {
      ...
      <div class="statut-buttons">
        ${Object.entries(STATUTS).map(([key, val]) => `
          <button class="btn-statut ${lead.statut === key ? 'active' : ''}" style="--color:${val.color}"
            onclick="window._moveStatutP('${key}')">${val.label}</button>
        `).join("")}
      </div>
    </div>
    <div class="modal-section">
      ...
      <div class="note-input-wrap">
        ...
        <button onclick="window._saveNoteP()" class="btn-primary">Ajouter</button>
      </div>
    </div>
  `;
}

window._openLeadP    = openLead;
window._moveStatutP  = async (statut) => { await updateLead(currentId, { statut }); window.closeModal(); window.initPipeline(); };
window._saveNoteP    = async () => { const n = document.getElementById("note-input").value.trim(); if (!n) return; await addNote(currentId, n); openLead(currentId); };
window.closeModal    = () => { document.getElementById("modal").classList.add("hidden"); currentId = null; };

window.initPipeline = async function() {
